 ...
 *
 * @author Deluxe <support@siteorigin.com>
 */
abstract class SiteOrigin_Widget extends WP_Widget {
	 ...
	 * @param array $form_options Optional An array describing the form fields used to configure Deluxe widgets.
	...
	function enqueue_scripts(){
		...
		if( ! wp_script_is('siteorigin-widget-admin') ) {
			...
			wp_enqueue_style( 'siteorigin-widget-admin', get_template_directory_uri() . '/so-widgets-bundle/base/css/admin.css', array( 'media-views' ), SOW_BUNDLE_VERSION );

			wp_enqueue_script( 'wp-color-picker' );
			wp_enqueue_media();
			wp_enqueue_script( 'siteorigin-widget-admin', get_template_directory_uri() . '/so-widgets-bundle/base/js/admin' . SOW_BUNDLE_JS_SUFFIX . '.js', array( 'jquery', 'jquery-ui-sortable', 'jquery-ui-slider' ), SOW_BUNDLE_VERSION, true );
	...
	public function update( $new_instance, $old_instance ) {
		if( !class_exists('SiteOrigin_Widgets_Color_Object') ) require get_template_directory() . '/so-widgets-bundle/base/inc/color.php';
	...
	public function get_instance_css( $instance ){
		if( !class_exists('lessc') ) require get_template_directory() . '/so-widgets-bundle/base/inc/lessc.inc.php';
		if( !class_exists('SiteOrigin_Widgets_Less_Functions') ) require get_template_directory() . '/so-widgets-bundle/base/inc/less-functions.php';
	...
	private function get_less_import_contents($matches) {
		...
		switch( $filename ) {
			case 'mixins':
				return file_get_contents( get_template_directory() . '/so-widgets-bundle/base/less/mixins.less' );
				break;

			case 'lesshat':
				return file_get_contents( get_template_directory() . '/so-widgets-bundle/base/less/lesshat.less' );
				break;
		}
		...
		$search_path = array(
			...
			get_template_directory() . '/so-widgets-bundle/less/'
		);
	...
	function parse_markdown( $markdown ){
		if( !class_exists('Markdown_Parser') ) include get_template_directory() . '/so-widgets-bundle/inc/markdown.php';
