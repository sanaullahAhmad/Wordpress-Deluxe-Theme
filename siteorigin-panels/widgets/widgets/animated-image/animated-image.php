...
class SiteOrigin_Panels_Widget_Animated_Image extends SiteOrigin_Panels_Widget  {
	...
	function enqueue_scripts(){
		...
		if(!$enqueued) {
			wp_enqueue_script('siteorigin-widgets-'.$this->origin_id.'-onscreen', get_template_directory() . '/siteorigin-panels/widgets/widgets/animated-image/js/onscreen.js', array('jquery'), SITEORIGIN_PANELS_VERSION);
			wp_enqueue_script('siteorigin-widgets-'.$this->origin_id, get_template_directory() . '/siteorigin-panels/widgets/widgets/animated-image/js/main.js', array('jquery'), SITEORIGIN_PANELS_VERSION);
			$enqueued = true;
		}
