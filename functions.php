...
function wp_bootstrap_custom_admin_footer() {
	...
	echo '<span id="footer-thankyou">Developed by <a href="http://peekinternational.com/" target="_blank">Peekinternational</a></span>. Built using <a href="http://themble.com/bones" target="_blank">Bones</a>.';
...
function wp_bootstrap_register_sidebars() {
  ...
  register_sidebar(array(
    ...
    'id' => 'customareaforcoding',

    'name' => 'Custom area for coding',

    'before_widget' => '<div id="%1$s" class="widget customareaforcoding col-md-12 col-sm-12 %2$s">',
...
//fxslider code function print_my_script() below will include the JS/CSS files only on those pages where the slider shortcode is actually used. 
...
// Slider Custom post type and taxonomy
...
//The following small function is responsible for creating a post meta value for each slide.
...
function simple_slider_shortcode($atts = null) {
	...
	if ( $the_query->have_posts() ) {
		...
		while ( $the_query->have_posts() ) {
			...
			if($slide_no==1){ $li_display = 'style="display:block"';}else {$li_display = 'style="display:none"';}
...

{
	...
	add_menu_page( "Theme Options", "Theme Options", 0, "theme-options", "theme_options", '', "10" );
...
include('siteorigin-panels/siteorigin-panels.php');



if(get_option( 'mydeluxkey')=='mydeluxkey')

{ 

include('mytheme_customize.php');
